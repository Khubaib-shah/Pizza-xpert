...
export default function CartSidebar({
  ...
  const handleSubmitOrder = async (e: FormEvent) => {
    ...
    if (!nameValidation.valid)
      validationErrors.name = nameValidation.error ?? "Full name is required";

    const phoneValidation = isPakistaniPhone(phone);
    if (!phoneValidation.valid)
      validationErrors.phone =
        phoneValidation.error ?? "Phone number is required";
            ...
            {cart.length > 0 ? (
              <>
                    ...
                    >
                      ...
                      <div className="flex-grow space-y-1.5 min-w-0">
                        ...
                        <div className="space-y-1 text-[10px] text-cream/50 bg-black/20 p-2 rounded-lg border border-white/5 leading-tight">
                          <div>
                            • <b className="text-cheese">SIZE:</b>{" "}
                            {item.customization.size}
                          </div>
                          <div>
                            • <b className="text-cheese">CRUST:</b>{" "}
                            {item.customization.crust}
                          </div>
                          <div>
                            • <b className="text-cheese">BASE:</b>{" "}
                          ...
                          {item.customization.extraToppings.length > 0 && (
                            <div className="truncate">
                              • <b className="text-tomato">TOPPINGS:</b>{" "}
                              {item.customization.extraToppings.join(", ")}
                            </div>
                          )}
                        </div>
                ...
                <div className="bg-charcoal-light/40 rounded-2xl p-4.5 border border-white/5 space-y-2.5 text-xs text-cream/80 font-medium">
                  ...
                  {deliveryFee > 0 && (
                    <div className="text-[10px] text-cheese/70 font-semibold uppercase text-center border border-dashed border-cheese/10 p-1.5 rounded bg-cheese/5">
                      💡 Add Rs{(25 - subtotal).toFixed(2)} more to secure FREE
                      delivery fee
                    </div>
                  )}
                ...
                >
                  ...
                  <div className="space-y-3.5 text-xs">
                    ...
                    />
                    {errors.api && (
                      <div className="text-xs text-tomato font-bold uppercase tracking-wider bg-burgundy/50 p-2 rounded-lg border border-tomato/50 mb-2">
                        {errors.api}
                      </div>
                    )}
                    ...
                    <div className="space-y-1">
                      <label className="text-cream/50 uppercase font-medium tracking-widest block">
                        Delivery Address (Door No / Floor / Apt)
                      </label>
                      <textarea
                        rows={2}
                      ...
                      />
                      {errors.address && (
                        <p className="text-[10px] text-tomato font-medium uppercase">
                          {errors.address}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <option value="Cash on Delivery (COD)">
                          Cash on Delivery (COD) / Card on Door
                        </option>
                        <option value="Debit Card Transfer (Visa/Mastercard)">
                          Secure Digital Card (Visa/MC Sim)
                        </option>
                        <option value="EasyPaisa Wallet">
                          EasyPaisa digital cash
                        </option>
                      </select>
                    </div>
                  </div>
              ...
              <div className="h-full flex flex-col justify-center items-center text-center py-12 px-4 space-y-4">
                ...
                <p className="font-sans text-xs text-cream/50 max-w-xs font-semibold">
                  You haven't added any woodfire masterpieces to your plate yet.
                  Swipe back to the signatures menu!
                </p>
                <button
                  onClick={onClose}
