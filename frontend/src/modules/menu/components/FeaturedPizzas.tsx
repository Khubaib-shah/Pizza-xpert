...
export default function FeaturedPizzas({
  ...
  const pizzas = pizzasRaw
    ? pizzasRaw.map((p: any) => ({ ...p, id: p._id || p.id }))
    : [];
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visibleEntries.length > 0) {
          onCategoryVisible(
            visibleEntries[0].target.id.replace("category-", ""),
          );
        }
      },
  ...
  const availableSizes = menuConfig?.sizes
  ...
  const availableCrusts = menuConfig?.crusts
  ...
  const availableSauces = menuConfig?.sauces
    ?.filter((s: any) => s.isAvailable)
    .map((s: any) => s.name) || [
  ...
  const availableToppings: Record<string, number> = menuConfig?.toppings
    ...
    : {
        "Sizzling Pepperoni": 200,
        "Artisan Sausage": 225,
        "Grilled Herb Chicken": 200,
        "Sautéed Mushrooms": 125,
        "Greek Feta Chunks": 150,
      };
  ...
  const [crust, setCrust] = useState<string>(
    availableCrusts[0] || "Classic Hand-Tossed",
  );
  const [sauce, setSauce] = useState<string>(
    availableSauces[0] || "Deep Tomato Marinara",
  );
  ...
  if (loading) {
    return (
      <section
        id="menu"
        className="relative bg-charcoal bg-grain py-20 px-4 md:px-6"
      >
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-cream/60 font-sans font-medium uppercase tracking-widest text-xs">
            Loading Menu...
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center space-y-4 mb-16">
          ...
          <h2 className="font-display text-4xl md:text-6xl font-medium text-cream tracking-wide uppercase leading-none">
            OUR <span className="text-cheese text-glow-gold">SIGNATURE</span>{" "}
            PIZZAS
          </h2>
          <div className="w-24 h-1.5 bg-cheese mx-auto rounded-full mt-2" />
          <p className="font-sans text-cream/90 text-sm md:text-base max-w-xl mx-auto font-medium">
            Meticulously engineered recipes carrying real craft history. Fully
            customizable with authentic Italian dough crust extensions.
          </p>
        </div>
        ...
        {searchMatchedPizzas.length > 0 ? (
            ...
            {apiCategories.length > 0
              ? apiCategories
                  ...
                  .map((category) => {
                    ...
                    const categoryPizzas = searchMatchedPizzas.filter(
                      (pizza) => pizza.category === slug,
                    );
                              ...
                              <div className="p-2 md:p-4 flex flex-col flex-grow justify-between space-y-4">
                                ...
                                <div className="space-y-1.5 text-left">
                                  <div className="flex items-center justify-between">
                                    ...
                                    >
                                      <span
                                        className={`w-1.5 h-1.5 rounded-full ${pizza.isVeg ? "bg-olive" : "bg-tomato"}`}
                                      />
                                    </span>
                                  </div>
                                  ...
                                  <p className="font-sans text-xs text-cream/90 line-clamp-3 leading-snug">
                                    {pizza.description}
                                  </p>
                                </div>
              ...
              : /* Fallback: no categories from API — render all pizzas in one flat grid */
                (() => {
                  const uniqueCategories = [
                    ...new Set(searchMatchedPizzas.map((p) => p.category)),
                  ];
                  return uniqueCategories.map((cat) => {
                    const catPizzas = searchMatchedPizzas.filter(
                      (p) => p.category === cat,
                    );
                    return (
                      <div
                          ...
                          {catPizzas.map((pizza) => (
                            ...
                            >
                              <div className="relative aspect-video overflow-hidden bg-black/40">
                                ...
                                {pizza.isSpicy && (
                                  <span
                                    className="absolute top-2 right-0 z-30 bg-burgundy text-cheese font-display text-xs inline-flex items-center gap-1 py-1 pr-2 pl-6 uppercase"
                                    style={{
                                      clipPath:
                                        "polygon(15% 0%, 100% 0%, 100% 100%, 15% 100%, 0% 50%)",
                                    }}
                              ...
                              <div className="p-4 flex flex-col flex-grow justify-between space-y-4">
                                <div className="space-y-1.5">
                                  <h3 className="font-display text-2xl font-medium text-white group-hover:text-cheese transition-colors uppercase">
                                    {pizza.name}
                                  </h3>
                                  <p className="font-sans text-xs text-cream/90 line-clamp-3">
                                    {pizza.description}
                                  </p>
                                </div>
                                <div className="flex gap-2">
                                  <button
                                    ...
                                    className="text-xs p-3 bg-white/5 hover:bg-white/15 border border-white/10 text-cream rounded-xl transition-all cursor-pointer"
                                  >
                                    customize
                                  </button>
                                  <button
                                    onClick={(e) => handleQuickAdd(e, pizza)}
                                    className="flex-1 btn-primary-anim bg-burgundy inline-flex justify-center text-cheese hover:bg-tomato hover:text-cream font-sans font-medium text-xs uppercase tracking-widest px-5 py-3 rounded-xl transition-all border border-tomato/20 cursor-pointer"
                                  >
                                    Rs {pizza.price.toFixed(2)}{" "}
                                    <Plus className="w-4 h-4 ml-2" />
                                  </button>
          ...
          <div className="text-center py-16 bg-charcoal-light/40 rounded-3xl border border-dashed border-white/10 max-w-lg mx-auto">
            ...
            <h3 className="font-display text-2xl font-medium text-cream mb-2 uppercase">
              NO PIZZAS FOUND MATCHING FILTER
            </h3>
            <p className="font-sans text-xs text-cream/60 px-6">
              Our master pizzaiolo hasn't cataloged details under those keyword
              specifications. Swipe back to another selection chip!
            </p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/85 z-50 flex items-end md:items-center justify-center md:p-6 backdrop-blur-md animate-fade-in">
          <div className="relative bg-charcoal-gray rounded-t-[32px] md:rounded-[24px] border-t md:border border-white/10 max-w-5xl w-full max-h-[90vh] md:max-h-[95vh] overflow-hidden card-pizza-shadow flex flex-col text-left animate-slide-up md:animate-none pb-safe">
            ...
            <div className="flex items-center justify-between p-5 md:px-8 border-b border-white/5 bg-charcoal-gray z-20">
              <h2 className="font-display text-xl !font-normal text-cream uppercase tracking-wide">
                Customize Order
              </h2>
              <button
                onClick={() => setCustomizingPizza(null)}
                className="p-2 bg-white/5 rounded-full text-cream/90 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto lg:overflow-hidden flex flex-col lg:flex-row">
              ...
              <div className="w-full lg:w-5/12 bg-charcoal-800 border-r border-white/5 p-4 md:p-6 lg:p-8 flex flex-col shrink-0">
                <div className="relative aspect-square w-full max-w-[140px] md:max-w-[200px] lg:max-w-none mx-auto mb-2 lg:mb-6">
                  ...
                  <div
                    className={`absolute inset-0 rounded-full blur-3xl opacity-20 transition-colors duration-500 ${extraCheese ? "bg-cheese" : "bg-tomato"}`}
                  />
                  <img
                  ...
                  {selectedToppings.length > 0 && (
                    <div className="absolute -right-4 top-1/4 z-20 space-y-2 animate-fade-in pointer-events-none">
                      {selectedToppings.slice(0, 3).map((top, i) => (
                        <div
                          key={top}
                          className="bg-charcoal/90 border border-white/10 text-cheese font-sans font-medium text-[9px] uppercase px-2 py-1 rounded-md shadow-lg"
                      ...
                      {selectedToppings.length > 3 && (
                        <div className="text-xs text-cream/50 ml-2">
                          ...and more
                        </div>
                      )}
                    </div>
                ...
                <div className="text-center lg:text-left mt-auto">
                  <span className="font-sans text-[9px] lg:text-[11px] font-medium text-tomato uppercase tracking-widest">
                    {customizingPizza.tagline}
                  </span>
                  <h3 className="font-display text-xl lg:text-3xl font-normal text-white uppercase mt-0.5 lg:mt-1">
                    {customizingPizza.name}
                  </h3>
                  <p className="font-sans text-xs text-cream/90 mt-2 font-medium leading-relaxed hidden lg:block">
                    {customizingPizza.description}
                  </p>

                  <div className="mt-3 lg:mt-6 pt-3 lg:pt-6 border-t border-white/10 flex items-center justify-between">
                    <span className="font-sans text-[10px] lg:text-xs font-medium text-cream/50 uppercase">
                      Total
                    </span>
                    <span className="font-display text-2xl lg:text-4xl font-medium text-cheese">
                      Rs.{calculateCustomizedPrice().toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full lg:w-7/12 p-4 md:p-6 lg:p-8 space-y-3 lg:space-y-4 overflow-y-auto lg:max-h-[calc(95vh-80px)]">
                {/* STEP 1: SIZE */}
                <div
                  className={`border rounded-2xl overflow-hidden transition-colors ${activeStep === 1 ? "border-cheese/50 bg-charcoal-light/40" : "border-white/5 bg-charcoal-light/20"}`}
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-6 h-6 rounded-full flex items-center justify-center font-medium text-xs ${activeStep === 1 ? "bg-cheese text-charcoal" : "bg-charcoal text-cream/50"}`}
                      >
                        1
                      </span>
                      <h4 className="font-sans font-medium text-sm uppercase text-cream tracking-wide">
                        Choose Pizza Size
                      </h4>
                    </div>
                    {activeStep !== 1 && (
                      <span className="font-sans text-xs font-medium text-cheese">
                        {size}
                      </span>
                    )}
                  </button>
                      ...
                      {availableSizes.map((s: string) => (
                        <button
                          ...
                          className={`relative p-3 rounded-xl border text-left transition-all cursor-pointer ${
                            size === s
                              ? "bg-burgundy/20 border-cheese ring-1 ring-cheese/50"
                              : "bg-charcoal border-white/5 hover:border-white/20"
                          }`}
                        >
                          {size === s && (
                            <Check className="absolute top-2 right-2 w-4 h-4 text-cheese" />
                          )}
                          <div
                            className={`font-sans font-medium text-xs uppercase ${size === s ? "text-cheese" : "text-cream/90"}`}
                          >
                            {s.split(" ")[0]}
                          </div>
                          <div className="font-sans text-[10px] text-cream/50 mt-1">
                            {s.split(" ")[1] || "Standard"}
                          </div>
                          <div className="font-mono text-[10px] font-medium text-cream/40 mt-2">
                            {(() => {
                              const sc = menuConfig?.sizes?.find(
                                (x: any) => x.name === s,
                              );
                              if (!sc || sc.priceModifier === 0)
                                return "Included";
                              return (
                                (sc.priceModifier > 0 ? "+" : "") +
                                "Rs." +
                ...
                {/* STEP 2: CRUST */}
                <div
                  className={`border rounded-2xl overflow-hidden transition-colors ${activeStep === 2 ? "border-cheese/50 bg-charcoal-light/40" : "border-white/5 bg-charcoal-light/20"}`}
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-6 h-6 rounded-full flex items-center justify-center font-medium text-xs ${activeStep === 2 ? "bg-cheese text-charcoal" : "bg-charcoal text-cream/50"}`}
                      >
                        2
                      </span>
                      <h4 className="font-sans font-medium text-sm uppercase text-cream tracking-wide">
                  ...
                  {activeStep === 2 && (
                    <div className="p-4 pt-0 grid grid-cols-1 sm:grid-cols-2 gap-3 animate-fade-in">
                      {availableCrusts.map((c: string) => {
                        const crustConfig = menuConfig?.crusts?.find(
                          (x: any) => x.name === c,
                        );
                        const isPremium = crustConfig?.isPremium || false;
                        const isRecommended =
                          crustConfig?.isRecommended || false;
                        return (
                          <button
                            key={c}
                            onClick={() => {
                          ...
                          >
                            {crust === c && (
                              <Check className="absolute top-3 right-3 w-4 h-4 text-cheese" />
                            )}
                            {isRecommended && (
                              <span className="inline-block px-2 py-0.5 bg-olive/20 text-olive text-[9px] font-medium uppercase rounded-md mb-2">
                                Recommended
                              </span>
                            )}
                            {isPremium && crustConfig?.priceModifier > 0 && (
                              <span className="inline-block px-2 py-0.5 bg-cheese/20 text-cheese text-[9px] font-medium uppercase rounded-md mb-2">
                                Premium +Rs.{crustConfig.priceModifier}
                              </span>
                            )}

                            <div
                              className={`font-sans font-medium text-xs uppercase ${crust === c ? "text-cheese" : "text-cream/90"}`}
                            >
                              {c}
                            </div>
                          </button>
                ...
                {/* STEP 3: SAUCE */}
                <div
                  className={`border rounded-2xl overflow-hidden transition-colors ${activeStep === 3 ? "border-cheese/50 bg-charcoal-light/40" : "border-white/5 bg-charcoal-light/20"}`}
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-6 h-6 rounded-full flex items-center justify-center font-medium text-xs ${activeStep === 3 ? "bg-cheese text-charcoal" : "bg-charcoal text-cream/50"}`}
                      >
                        3
                      </span>
                      <h4 className="font-sans font-medium text-sm uppercase text-cream tracking-wide">
                    ...
                    <div className="p-4 pt-0 grid grid-cols-2 gap-3 animate-fade-in">
                      {availableSauces.map((sa: string) => (
                        ...
                        >
                          ...
                          <div
                            className={`font-sans font-medium text-[11px] uppercase pr-6 ${sauce === sa ? "text-cheese" : "text-cream/90"}`}
                          >
                            {sa}
                          </div>
                        </button>
                ...
                <div
                  className={`border rounded-2xl overflow-hidden transition-colors ${activeStep === 4 ? "border-cheese/50 bg-charcoal-light/40" : "border-white/5 bg-charcoal-light/20"}`}
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-6 h-6 rounded-full flex items-center justify-center font-medium text-xs ${activeStep === 4 ? "bg-cheese text-charcoal" : "bg-charcoal text-cream/50"}`}
                      >
                        4
                      </span>
                      <h4 className="font-sans font-medium text-sm uppercase text-cream tracking-wide">
                  ...
                  {activeStep === 4 && (
                    <div className="p-4 pt-0 space-y-6 animate-fade-in">
                      ...
                      <div>
                        <h5 className="font-sans text-[10px] font-medium text-cream/50 uppercase tracking-widest mb-3">
                          Premium Base Upgrade
                        </h5>
                        <label className="flex items-center justify-between p-4 bg-charcoal border border-white/5 rounded-xl cursor-pointer hover:border-white/20 transition-colors">
                          <div className="flex items-center gap-4">
                            <div
                              className={`relative w-12 h-6 rounded-full transition-colors ${extraCheese ? "bg-cheese" : "bg-white/10"}`}
                            >
                              <div
                                className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${extraCheese ? "translate-x-6" : "translate-x-0"}`}
                              />
                            </div>
                            <div>
                              <div
                                className={`font-sans font-medium text-xs uppercase ${extraCheese ? "text-cheese" : "text-cream"}`}
                              >
                                Double Mozzarella Drip
                              </div>
                              <div className="font-sans text-[10px] text-cream/50 mt-0.5">
                                Extra heavy layer of Wisconsin whole milk cheese
                              </div>
                            </div>
                          </div>
                          <span className="font-mono text-xs font-normal text-cheese">
                            +Rs.{extraCheesePrice}
                          </span>
                        </label>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-col gap-2">
                          {Object.entries(availableToppings).map(
                            ([top, topPrice]) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`relative p-3.5 px-4 rounded-xl border text-left transition-all cursor-pointer flex items-center justify-between ${
                                    isSelected
                                      ? "bg-burgundy/20 border-cheese ring-1 ring-cheese/50"
                                      : "bg-charcoal border-white/5 hover:border-white/20"
                                  }`}
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div
                                      className={`font-sans font-medium text-[11px] uppercase ${isSelected ? "text-cheese" : "text-cream/90"}`}
                                    >
                                      {top}
                                    </div>
                                  </div>
