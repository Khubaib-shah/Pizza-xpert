...
export default function PremiumHorizontalMenu({ onAddSpecialDealToCart }: PremiumHorizontalMenuProps) {
  ...
  const deals: Deal[] = (landingContent?.deals || []).map((d: any) => ({ ...d, id: d._id || d.id }));
    ...
    <section className="bg-charcoal py-24 px-4 md:px-6 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="font-sans text-cream/80 text-sm max-w-lg mx-auto font-medium tracking-wider">
                ...
                <div className={`w-48 md:w-72  lg:w-80  absolute bottom-0 ${isReverse ? 'right-0' : 'left-0'} z-10`}>
                  {deal.image ? (
                    <CloudinaryImage
                      src={deal.image}
                      alt={deal.title}
                      className="w-full h-auto drop-shadow-[0_20px_30px_rgba(0,0,0,0.6)] filter scale-110 md:scale-125 translate-y-6"
