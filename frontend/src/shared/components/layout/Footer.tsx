...
export default function Footer({
  ...
  return (
    <footer className="relative bg-charcoal-black bg-grain text-cream/90 py-16 px-4 md:px-6 overflow-hidden border-t border-white/5">
      <div className="max-w-7xl mx-auto space-y-12 relative z-10 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8">
          ...
          <div className="space-y-5">
            ...
            <p className="font-sans text-xs text-cream/50 max-w-sm leading-tight">
              Serving the highest-density, woodfire-baked pizza marvels in the
              urban sectors. Our recipe rules are unbreakable.
            </p>
            ...
            <ul className="space-y-2.5 font-sans text-xs font-medium uppercase tracking-wider">
              <li>
                <button
                  ...
                  className="hover:text-cheese transition-colors cursor-pointer text-left"
                >
                  HOMEPAGE PORTAL
                </button>
              </li>
              ...
              <li>
                <button
                  onClick={() => onScrollToElement("deals")}
                  className="hover:text-cheese transition-colors cursor-pointer text-left"
                >
              ...
              <li>
                <button
                  onClick={() => onScrollToElement("about")}
                  className="hover:text-cheese transition-colors cursor-pointer text-left"
                >
                  ABOUT OUR BAKERS
                </button>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 font-sans text-[10px] uppercase font-medium">
              <a
                href="/"
                className="hover:text-cheese transition-colors text-cream/50"
              >
                Best Pizza Home
              </a>
              <a
        ...
        <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-sans font-medium text-cream/40 uppercase tracking-wider">
          <div>
            © {new Date().getFullYear()} PIZZA XPERT BRAND LLC. EXPERTISE IN
            EVERY SLICE!
          </div>

          {/* Payment currencies */}
          <div className="flex items-center gap-2">
            {["Visa", "Mastercard", "EasyPaisa", "COD (Hand Cash)"].map(
              (pm, idx) => (
                <span
                  key={idx}
