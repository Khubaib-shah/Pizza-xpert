...
import { X, Flame, Bike, CheckCircle2, Clock, MapPin, Sparkles, Loader } from 'lucide-react';
...
export default function OrderTracker({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeRemaining((prev) => {
        ...
        if (demoSpeedUp) {
          if (nextTime > 1350 && nextTime <= 1650 && currentStage === 'placed') {
            setCurrentStage('preparing');
            if (onAdvanceStage) onAdvanceStage(order.id, 'preparing');
          } else if (nextTime > 900 && nextTime <= 1350 && currentStage === 'preparing') {
            setCurrentStage('baking');
            if (onAdvanceStage) onAdvanceStage(order.id, 'baking');
          } else if (nextTime > 0 && nextTime <= 900 && currentStage === 'baking') {
  ...
  const stages: { stage: OrderStage; label: string; text: string; icon: string }[] = [
    {
      stage: 'placed',
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden font-sans select-none flex items-center justify-center p-4">
      ...
      <div className="relative bg-charcoal bg-grain rounded-[28px] border border-white/10 max-w-xl w-full p-6 md:p-8 card-pizza-shadow text-left">

        {/* Modal Exit */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 p-2.5 bg-black/60 rounded-full text-cream hover:text-cheese transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="space-y-6">
          ...
          <div className="border-b border-white/5 pb-4.5 flex flex-wrap items-center justify-between gap-3 text-left">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-burgundy/80 text-cheese font-sans text-[10px] font-black tracking-widest rounded-full uppercase border border-tomato/10">
            ...
            <div className="bg-charcoal-light border border-white/5 rounded-2xl p-3 flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="font-mono text-base font-black text-cheese mt-0.5">{timeFormatted}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            <h4 className="font-sans font-black text-xs uppercase text-cream tracking-wider">
            ...
            <div className="space-y-4 relative pl-3.5">
              ...
              {stages.map((st, sIdx) => {
                ...
                return (
                  <div key={st.stage} className="flex gap-4 items-start text-xs font-sans relative z-10 select-none">
                    {/* Circle icon bullet */}
                    <div
                    ...
                    <div className="flex-grow flex gap-3 text-left">
                      <span className="text-base leading-none select-none">{st.icon}</span>
                      <div className="leading-tight">
                        <div
                          className={`font-black uppercase tracking-wider ${isActive ? 'text-cheese font-black' : isCompleted ? 'text-white/80 font-black' : 'text-cream/35'
                            }`}
                        >
                          {st.label}
                        </div>
                        <p
                          className={`text-[10px] font-medium leading-tight mt-0.5 ${isActive ? 'text-cream/90' : isCompleted ? 'text-cream/50' : 'text-cream/20'
                            }`}
                        >
                          {st.text}
                        </p>
          ...
          <div className="bg-charcoal-light/80 p-4.5 rounded-2xl flex items-center gap-4 border border-white/5 relative overflow-hidden text-left">
            ...
            {currentStage === 'placed' && (
              <div className="w-12 h-12 rounded-full bg-burgundy/50 text-cheese flex items-center justify-center text-xl animate-pulse">📝</div>
            )}
            {currentStage === 'preparing' && (
              <div className="w-12 h-12 rounded-full bg-burgundy/50 text-cheese flex items-center justify-center text-xl">🥣</div>
            )}
            {currentStage === 'baking' && (
              <div className="w-12 h-12 rounded-full bg-tomato/20 text-tomato flex items-center justify-center text-xl animate-bounce">🔥</div>
            )}
            {currentStage === 'delivering' && (
              <div className="w-12 h-12 rounded-full bg-cheese/10 text-cheese flex items-center justify-center text-xl animate-bounce"><Bike className="w-6 h-6 text-cheese" /></div>
            )}
            {currentStage === 'delivered' && (
              <div className="w-12 h-12 rounded-full bg-olive/20 text-olive flex items-center justify-center text-xl"><CheckCircle2 className="w-6 h-6 text-olive" /></div>
            )}
            ...
            <div className="flex-grow z-10">
              ...
              <p className="font-sans text-[11px] font-bold text-cream/60 uppercase tracking-widest mt-0.5 leading-tight">
                {currentStage === 'placed' && 'Processing transaction and queuing into chefs baking ledger...'}
                {currentStage === 'preparing' && 'Kneading gourmet sourdough base and adding selected dressings...'}
                {currentStage === 'baking' && 'Cooking at extreme temperature (850 degrees) in wood fired brick oven...'}
                {currentStage === 'delivering' && 'Moto Courier speeding through grid lock. Pizza kept extremely cozy...'}
                {currentStage === 'delivered' && 'Cargo delivered. Slices unlocked. Grab plates immediately!'}
              </p>
            </div>
