...
export default function GalleryManager({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setErrorModal({
        title: "Upload Failed",
        message: "There was a problem uploading your images. Please try again.",
      });
    } finally {
      setUploading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setErrorModal({
        title: "Delete Failed",
        message: "There was a problem deleting this image. Please try again.",
      });
    } finally {
      setDeleting(false);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {filteredGallery.map((img) => (
          <div
            key={img._id}
            className="group relative bg-charcoal border border-charcoal-border rounded-xl overflow-hidden hover:border-cheese transition-all"
          >
            ...
            <div className="p-3">
              <p className="text-[10px] font-mono text-cream/90 truncate">
                {img.filename}
              </p>
              <p className="text-[9px] text-cream/40 mt-1 uppercase">
                {img.folder}
              </p>
            </div>
          </div>
