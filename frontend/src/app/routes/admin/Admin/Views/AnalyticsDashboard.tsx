...
export default function AnalyticsDashboard({
  ...
  const avgOrderValue =
    ordersHandled > 0 ? Math.round(revenue / ordersHandled) : 0;

  // Calculate Donut Categories
  const totalItems = orders.length || 1;
  const loadedCount = orders.filter((o) =>
    o.items.toLowerCase().match(/loaded|spicy|bbq/),
  ).length;
  const classicCount = orders.filter((o) =>
    o.items.toLowerCase().match(/classic|margherita|pepperoni/),
  ).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          {
            title: "KARACHI TOTAL REVENUE",
            value: `Rs. ${revenue.toLocaleString()}`,
            change: "Live Sync",
            glow: "shadow-[0_4px_20px_rgba(245,177,9,0.15)]",
            barGlow: "bg-cheese",
          },
          {
            title: "TOTAL ORDERS HANDLED",
            value: ordersHandled.toString(),
            change: "Live Sync",
            glow: "shadow-[0_4px_20px_rgba(34,197,94,0.15)]",
            barGlow: "bg-emerald-500",
          },
          {
            title: "ACTIVE COURIERS",
            value: `${activeCouriers} Riders`,
            change: "GPS Tracking Active",
            glow: "shadow-[0_4px_20px_rgba(59,130,246,0.15)]",
            barGlow: "bg-blue-500",
          },
          {
            title: "NEW LOGGED VISITORS",
            value: `${newSignups} Signups`,
            change: "Live Sync",
          ...
          <div
            key={idx}
            className={`bg-charcoal-border/70 backdrop-blur-md rounded-2xl p-4 border border-white/5 relative overflow-hidden flex flex-col justify-between h-28 hover:border-cheese/30 transition-all ${card.glow}`}
          >
            <div className="absolute top-0 left-0 w-1 h-full col-scale" />
            <div>
              <span className="text-[10px] uppercase tracking-widest font-mono text-cream/60 font-medium">
                {card.title}
              </span>
              <h3 className="text-3xl font-display font-medium text-white mt-1">
                {card.value}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-charcoal border border-charcoal-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-xs uppercase tracking-widest font-medium text-white">
              KARACHI REVENUE TIMELINE (LAST 30 DAYS)
            </h4>
            <span className="text-xs text-cheese font-mono font-medium">
              AVG VALUE: Rs. {avgOrderValue.toLocaleString()}/order
            </span>
          </div>
          {/* SVG graph */}
          <div className="relative h-64 w-full">
            <svg
              viewBox="0 0 500 200"
              className="w-full h-full text-cheese"
        ...
        <div className="lg:col-span-4 bg-charcoal border border-charcoal-border rounded-2xl p-5 flex flex-col justify-between">
          <div>
            <h4 className="text-xs uppercase tracking-widest font-medium text-white mb-4">
              ORDERS BY CATEGORY
            </h4>
            {/* Fake Donut SVG */}
            <div className="flex items-center justify-center h-40">
              <div className="relative w-32 h-32">
                <svg
                  viewBox="0 0 36 36"
                  className="w-full h-full circular-chart"
                >
                  <path
                    className="circle-bg"
                    stroke="var(--color-charcoal-muted)"
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-center">
                  ...
                  <span className="text-[8px] text-cream/40 uppercase font-medium">
                    ORDERS
                  </span>
                </div>
              </div>
          ...
          <div className="space-y-1.5 text-xs text-cream/90">
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-burgundy" /> Solid
                Loaded
              </span>
            ...
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-cheese" /> Woodfire
                Classics
              </span>
            ...
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500" /> Cheesy
                Herbals
              </span>
      ...
      <div className="bg-charcoal border border-charcoal-border rounded-2xl p-5">
        <h4 className="text-xs uppercase tracking-widest font-medium text-white mb-3">
          KARACHI PEAK ORDERING HEATMAP (7 DAYS × 24 HOURS GRID)
        </h4>
        <div className="space-y-1">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day, idx) => (
            <div key={idx} className="flex items-center gap-1">
              <span className="w-8 text-[10px] font-mono text-cream/40">
                {day}
              </span>
      ...
      <div className="bg-charcoal border border-charcoal-border rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-charcoal-border flex justify-between items-center">
          <h4 className="text-xs uppercase tracking-widest font-medium text-white">
            RECENT ORDERS DESPATCH TRAIL
          </h4>
          <button
            onClick={() => navigate("/admin/orders")}
            className="text-xs text-cheese font-medium uppercase hover:underline"
          >
            Full Registry →
          </button>
        </div>
        <table className="w-full text-left border-collapse text-xs">
          ...
          <tbody className="divide-y divide-charcoal-800">
            {orders.slice(0, 3).map((o) => (
              <tr
                key={o.id}
                className="hover:bg-charcoal-800/30 transition-colors font-mono"
              >
                <td className="p-3 font-medium text-cheese">{o.id}</td>
                <td className="p-3 text-white font-sans font-medium">
                  {o.customer}
                </td>
                <td className="p-3 text-cream/90 font-sans">{o.items}</td>
